...
const { getClaudeInstance, BASE_PROMPT } = require('./anthropic_llm.js');
...
async function handleStackTrace(message, sender, channel, msgobj) {
    try {
        ...
        const anthropicClient = getClaudeInstance();
        ...
        const { model, response } = await anthropicClient.analyzeStackTrace(prompt);
        ...
        // send the results
        await sendStackTraceResponse(msgobj, response, tokens);
    } catch (error) {
...
async function sendStackTraceResponse(message, analysisResult, tokens) {
    ...
    if (includeTokenCounts) {
        responseEmbed.addFields(
            ...
            { name: 'Prompt Length', value: prompt.length.toString() },
            { name: 'Response Length', value: response.length.toString() },
        );
    }
